...
- **Rajagopal et al., 2025 — "Leveraging Blockchain and FL in Edge-Fog-Cloud Computing for ECG Data in IoT"** Journal of Network and Computer Applications, vol. 233 This is the most directly comparable paper. It's FL + blockchain for ECG data across IoT edge devices, exactly your setup. It uses a three-tier edge-fog-cloud architecture and smart contracts for decision-making. Key difference: they use a fog layer intermediary; your system is flat (clients → server → Ethereum directly). No per-round immutable voting mechanism like yours. https://doi.org/10.1016/j.jnca.2024.104037
- **Otoum et al., 2024 — "Enhancing Heart Disease Prediction with Federated Learning and Blockchain Integration"** Future Internet, 16(372) Blockchain-integrated FL for cardiac data. Uses blockchain to secure model updates but doesn't target ECG superclass classification on PTB-XL and doesn't address class imbalance with ROS+RUS. No on-chain voting filter. https://doi.org/10.3390/fi16100372

#### Category 2 — FL on ECG / PTB-XL (same dataset, no blockchain)
- **Jimenez et al., 2024 — "Application of FL Techniques for Arrhythmia Classification Using 12-Lead ECG Signals"** arXiv:2208.10993v3 This is already your primary baseline — you directly adopted their ROS+RUS balancing strategy. They achieve F1 scores in the FL Non-IID setting on 12-lead ECG. No blockchain, no IoT edge deployment, no immutable ledger. This is your clearest comparison point for classification performance. https://doi.org/10.48550/arXiv.2208.10993
- **FL-LSTM (Explainable FL), 2025 — "Explainable Federated Learning for Multi-Class Heart Disease Diagnosis via ECG Fiducial Features"** Diagnostics, MDPI, Dec 2025 Achieves 92% accuracy, 99% AUC, and 91% F1 score across three heterogeneous ECG datasets using an FL-LSTM framework, with SHAP and LIME for interpretability. MDPI No blockchain, no IoT hardware, no class-imbalance handling. Your model is architecturally simpler but adds the full blockchain audit layer they lack. https://doi.org/10.3390/diagnostics15243110
- **FL-GAF (Gramian Angular Field), 2025 — "Federated Learning with GAF for Privacy-Preserving ECG Classification on Heterogeneous IoT Devices"** IEEE ComComAp 2025 Transforms 1D ECG signals into 2D Gramian Angular Field images and deploys FL across heterogeneous IoT devices including a Raspberry Pi 4, achieving 95.18% classification accuracy. arXiv This is especially important for your prof's IoT vision — they actually ran it on a Pi. No blockchain. Your novelty over this paper is the entire blockchain audit layer.

#### Category 3 — Blockchain + FL on IoT (same infrastructure, different domain)
- **Leash-FL, 2025 — "Lightweight ECC-Based Self-Healing FL Framework for Secure IIoT Networks"** Sensors, MDPI, Nov 2025 Records all screening outcomes immutably on a lightweight blockchain — each record includes the hash of the submitted update, pseudonym, timestamp, and verdict — while model tensors remain off-chain to preserve efficiency. MDPI Very close to your architecture philosophically. They use ECC authentication and a self-healing mechanism. Key difference: industrial IoT domain, not medical ECG. Their blockchain is permissioned/private; yours is public Ethereum. https://doi.org/10.3390/s25226867
- **FBCI-SHS, 2025 — "Blockchain Framework with IoT Device using FL for Sustainable Healthcare"** Scientific Reports, Jul 2025 Uses PBFT consensus and cryptographic keys unique to each IoT device to sign model updates, with smart contracts checking authenticity and validity before posting to the blockchain ledger. Nature Healthcare domain + FL + blockchain + IoT — very aligned. Uses PBFT (heavier than your PoA) and doesn't handle ECG superclass imbalance.

#### Category 4 — PTB-XL Benchmarks (performance reference only)
- **Strodthoff et al. (PTB-XL benchmark)** — The standard reference for what's achievable on PTB-XL superclass. Achieves macro-AUC scores of 0.93 for diagnostic categories using ResNet- and Inception-based networks. PubMed Central This is the ceiling you're compared against for pure classification performance.
- **MRF-CNN on PTB-XL** — Achieves F1 score of 0.72 and AUC of 0.93 in superclass classification on PTB-XL. PubMed Central A direct F1 comparison target for your model.
